...
stock = input("Enter the symbol symbol : ") #Asks for symbol ticker
...
while stock != "quit": #Runs this loop until user enters 'quit' (can do many stocks in a row)
  ...
  df = pdr.get_data_yahoo(stock, start, now) #Fetches symbol price data, saves as data frame

  sma = int(input("Enter the sma : ")) #Asks for symbol ticker
  ...
  stock = input("Enter the symbol symbol : ") #Asks for new symbol
